...
function afficherEvent ($event){
		...
		echo "id promo: ".$event->getidPromo()."<br>";
	}
	function calculerSalaire($event){
		echo $event->getDescription() * $event->getDescription();
	}
	function ajouterEvent($event){
		$sql="insert into evenement (id,nom,dateDebut,dateFin,description,idPromo) values (:id, :nom,:dateD,:dateF,:description, :idPromo)";
	...
	function modifierEvent($event,$id){
		$sql="UPDATE evenement SET id=:idd, nom=:nom,dateDebut=:dateD,dateFin=:dateF,description=:description,idPromo=:idPromo WHERE id=:id";
...
try{		
        ...
        $description=$event->getDescription();
        $idPromo=$event->getidPromo();
		$datas = array(':idd'=>$idd, ':id'=>$id, ':nom'=>$nom,':dateD'=>$datedebut,':dateF'=>$datefin,':description'=>$description, ':idPromo'=>$idPromo);
