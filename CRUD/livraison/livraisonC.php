...
function afficherLivraison ($livraison){
	...
	function ajouterLivraison($livraison){
		$sql="insert into livreur values (:id, :destination,:dated,:datef,:idclient,:idarticle)";
	...
	function supprimerLivraison($id){
		$sql="DELETE FROM Livraison where Id-livraison= :id";
		$db = config::getConnexion();
        $req=$db->prepare($sql);
		$req->bindValue(':id',$id);
		try{
	...
	function modifierLivraison($livreur,$id){
		$sql="UPDATE Livraison SET Id-livraison=:id, Destination=:destination, Date-debut=:dated, Date-fin=:datef, Id-client=:idclient, Id-article =:id-article WHERE Id-livraison=:id";
...
try{		
  ...
  print_r($datas);
	...
	function recupererLivraison($id){
		$sql="SELECT * from livraison where Id-livraison=:id";
		$db = config::getConnexion();
		try{
		$liste=$db->query($sql);
